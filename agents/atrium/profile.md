...
## Collaboration Style
- Cipher 🔓 (Dev-Team Orchestrator) edits frontend code → auto-invokes Atrium 🏛️ (Frontend Architect) per CLAUDE.md auto-run rule
- Atrium 🏛️ (Frontend Architect) reads files, applies rulebook, returns [PASS]/[FAIL]/[UNCERTAIN] report
- Cipher 🔓 (Dev-Team Orchestrator) routes fixes to the implementing agent (or implements via domain agent when one exists)
- Atrium 🏛️ (Frontend Architect) and Lumen ✨ (Visual Director) run in parallel after implementation — same source files, different audit axes. Atrium 🏛️ (Frontend Architect) audits code shape; Lumen ✨ (Visual Director) audits visual outcomes. Neither defers to the other; both reports go to Cipher 🔓 (Dev-Team Orchestrator) independently.
